...
function SellerJoin(props) {
  ...
  let onClickConfirmButton = () => {
    axios.post(`${process.env.REACT_APP_API_URL}/sellers`, { email: email, streetAddress: address, detailAddress: detailAddress, zipCode: zoneCode, name: name, companyName: companyName, password: pw, phone: phone1 + '-' + phone2 + '-' + phone3 })
      .then((result) => {
        alert('회원가입 완료');
  ...
  useEffect(() => {
    if (companyName.trim() !== '', name.trim() !== '' && address.trim() !== '' && detailAddress.trim() !== '' && zoneCode.trim() !== '' && phone1.trim() !== '' && phone2.trim() !== '' && phone3.trim() !== '' && emailValid && pwValid && pwConfirmValid) {
      setNotAllow(false);
      return;
    }
    setNotAllow(true);
  }, [companyName, name, address, detailAddress, zoneCode, phone1, phone2, phone3, emailValid, pwValid, pwConfirmValid]);
        ...
        <Row>
          <Col>
            <Form className='login-ContentWrap'>
              ...
              <Form.Group className="mb-3 text-start" controlId="formBasicCompanyName">
                <Form.Label className='login-InputTitle'>회사명</Form.Label>
